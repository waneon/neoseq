...
/**
 * One `MoveBlock` per selected root, in document order.
 *
 * The core reads `index` as a position among the target parent's *other*
 * children — which, while the group is only part-way moved, still include the
 * movers that have not gone yet. Taking `start + offset` therefore lands the
 * group in the wrong order whenever a mover currently sits in front of the row it
 * is supposed to follow. So the plan walks an anchor instead: each root goes
 * immediately after the last one placed, and the sibling list is simulated
 * forward exactly as the core will rewrite it.
 */
export function movePlan(
  rows: readonly OutlineRow[],
  roots: readonly OutlineRow[],
  target: DropTarget,
): MoveStep[] {
  let children = rows
    .filter((row) => row.parentId === target.parentId)
    .map((row) => row.block.id);
  let anchor = target.afterId;
  const steps: MoveStep[] = [];
  for (const root of roots) {
    const others = children.filter((id) => id !== root.block.id);
    const index = anchor === null ? 0 : others.indexOf(anchor) + 1;
    steps.push({ blockId: root.block.id, parent: target.parentId, index });
    others.splice(index, 0, root.block.id);
    children = others;
    anchor = root.block.id;
  }
  return steps;
}

/**
 * The order group indent and group outdent have to run in.
 *
 * Indent is document order, and needs no rearranging at all: each `IndentBlock`
 * re-reads the previous sibling, and the new parent stays put at the position
 * above, so repeated single indents land the whole run under one parent — and the
 * core's own "first sibling cannot be indented" still speaks for the group.
 *
 * Outdent has to run backwards. `OutdentBlock` puts a block immediately after its
 * former parent, so outdenting the last root first leaves the earlier ones room to
 * land in front of it rather than behind.
 */
export function outdentOrder(roots: readonly OutlineRow[]): OutlineRow[] {
