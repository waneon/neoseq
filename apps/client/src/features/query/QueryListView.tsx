      ...
      >
        {rowWindow.rows.map((row) => {
          ...
          return (
            ...
            >
              <BlockBody className="query-list-text" taskStatus={taskStatus} markCount={markCount}>
                {markCount > 0 && (
...
export function QueryGenericListView({
  ...
  const { message } = useI18n();
  const lead =
    columns.find((column) => column.source?.kind === "content") ??
...
function GenericResultRow({
  ...
  const text =
    (lead ? cellText(row.values[lead.variable], lead, context) : "") ||
      ...
      gutter={
        entity && context.onOpen ? (
          <button
            ...
            className="outline-bullet"
            aria-label={
              text ? message("query.openResult", { name: text }) : message("query.openEmptyResult")
            }
