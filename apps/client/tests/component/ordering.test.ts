...
import { ORDER_STEP, moveWrites, nextOrder, place, respace } from "../../src/entities/ordering";
import {
  FAVOURITE_KEY,
  FAVOURITE_ORDER_KEY,
  favourites,
  ...
  const fields: PropertyField[] = [
    {
      ...
      value_type: "checkbox",
      cardinality: "single",
      values: [{ type: "checkbox", value: true }],
    },
  ];
  if (order !== undefined) {
    fields.push({
...
describe("the starred list", () => {
  it("reads pages and tags as one list, arranged first and named after", () => {
    ...
    expect(favourites(snapshot, compare).map((entry) => entry.name)).toEqual([
      "Zebra",
      "Reading",
      "Apple",
    ]);
  ...
  it("moves one row and writes one position", () => {
    ...
    expect(moveFavourite(ordered, reading, ordered[1])).toEqual([
      { entry: reading, order: ORDER_STEP / 2 },
    ]);
    // Dropped past the end, with nothing following to land before.
    expect(moveFavourite(ordered, ordered[0], null)).toEqual([
      { entry: ordered[0], order: 3 * ORDER_STEP },
    ]);
  });
  ...
  it("keeps a page and a tag apart when they share an id", () => {
    ...
    expect(moveFavourite(ordered, ordered[1], ordered[0])).toEqual([
      { entry: ordered[1], order: -ORDER_STEP },
    ]);
  });
});
