...
checks `navigator.languages` in order and follows BCP 47 fallback from the exact
tag to language-script and base language (`ko-KR` matches `ko`), then falls back
to `en`. It follows the browser's
...
directions, localized label keys, and temporal language packs. Adding a locale starts there. Source catalogs
...
To add a locale, add one manifest entry and one complete catalog, add or select a
temporal language pack, regenerate the typed contract, and extend focused locale and browser
