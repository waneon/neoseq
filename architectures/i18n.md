...
`pnpm --filter @neoseq/client i18n:generate` refreshes the generated module. Client
`check` and `build` use `--check`, so stale generated output fails without modifying
the checkout.
...
set of current validation diagnostics is also mapped to dedicated keys; unknown or
unsafe core detail is replaced by a localized generic reason rather than shown as
English. The original diagnostic remains available to non-UI diagnostics. Future
