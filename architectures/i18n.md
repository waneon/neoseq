...
`apps/dashboard/src/i18n/` owns a smaller runtime over the same boundary: typed
...
record alongside unrelated local preferences. The dashboard stores its independent
preference under `neoseq.dashboard.locale.v1`; it never persists the memory-only admin
...
when stale. Web tasks depend on `i18n:check`, and `outputs.neoseq-client` and
`outputs.neoseq-dashboard` check their respective generated contracts, so verification
and production builds reject stale output without modifying the checkout.
...
The dashboard maps stable HTTP status classes to its own catalog and never presents
