...
impl GraphCore {
    ...
    fn validate(&self, command: &Command) -> Result<(), CoreError> {
        match command {
            ...
            | Command::RemovePageDefault { page_id, .. } => {
                self.require_page(page_id)?;
            }
    ...
    fn apply(
        ...
        match command {
            ...
            Command::RenamePage { page_id, title } => {
                let bag = self.page_bag(page_id, "properties")?;
                set_single(
                    &bag,
                    &key("page.title"),
                    &PropertyValue::String(title.clone()),
                )?;
            }
            Command::DeletePage { page_id } => {
                let bag = self.page_bag(page_id, "properties")?;
            ...
            Command::RestorePage { page_id } => {
                self.page_bag(page_id, "properties")?
            ...
            Command::SetPageDefault {
                ...
                set_single(&self.page_bag(page_id, "defaults")?, key, value)?;
            }
            Command::RemovePageDefault { page_id, key } => {
        ...
        if !existed {
            ...
            if let Some(title) = title {
                set_single(
                    &properties,
                    &key("page.title"),
                    &PropertyValue::String(title.to_owned()),
                )?;
            }
    ...
    fn require_live_page(&self, page_id: &PageId) -> Result<LoroMap, CoreError> {
        let page = self.require_page(page_id)?;
        if bag_contains_key(
            &page.ensure_mergeable_map("properties")?,
            &key("system.deleted-at"),
        ) {
...
fn semantic_name(command: &Command) -> &'static str {
    match command {
        ...
        Command::SetPageDefault { .. } | Command::RemovePageDefault { .. } => "PageDefaultsChanged",
...
fn block_snapshot(
    ...
    let meta = outline.get_meta(node)?;
    let markdown = match meta.get("markdown") {
        Some(ValueOrContainer::Container(Container::Text(text))) => text.to_string(),
        _ => {
            quarantined.push(format!("block:{node}:missing-markdown"));
            String::new()
        }
    };
...
mod tests {
    ...
    fn model_round_trip_all_values_unknown_and_tag_defaults() {
        ...
        assert_eq!(
            entries
                .iter()
                .filter(|entry| entry.key.as_str() == "tag")
                .count(),
            1
        );
    ...
    fn model_local_undo_does_not_remove_imported_remote_change() {
        ...
        let home = snapshot
            ...
            .unwrap();
        assert!(home.properties.iter().any(|entry| {
            entry.key.as_str() == "page.title"
                && entry.value == PropertyValue::String("Home".into())
        }));
    }
    ...
    fn model_page_restore_and_invalid_projection_quarantine() {
        ...
        .unwrap();
        set_single(
            &core.page_bag(&page(), "defaults").unwrap(),
            &key("page.title"),
            &PropertyValue::String("invalid default".into()),
        )
        .unwrap();
        core.doc.commit();
        ...
        assert!(snapshot.pages[0].defaults.is_empty());
        assert!(
        ...
        assert_eq!(snapshot.quarantined.len(), 2);
    }
}
