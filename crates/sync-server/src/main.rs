...
use sync_server::{AppState, Metrics, PgIdentity, PgStore, RoomConfig, RoomManager, router};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if env::args_os().nth(1).is_some() {
        return Err(io::Error::other("sync-server does not accept command arguments").into());
    }
